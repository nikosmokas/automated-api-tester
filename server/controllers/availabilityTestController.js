...
const performAvailabilityTest = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: 'Availability tests completed and results saved', results: testResults });
    } catch (error) {
...
const getAvailabilityTestResults = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: 'Failed to fetch availability test results' });
  }
};
